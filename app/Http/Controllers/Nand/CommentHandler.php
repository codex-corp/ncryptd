...
class CommentHandler extends Eloquent
{
    ...
    function __construct($keep_first)
    {
        parent::__construct();
    ...
    function RemoveComments(&$contents)
    {
        ...
        if (in_array('//', $StdReplaceComments)) {
            ...
            $contents = preg_replace_callback("/[ \t\n]+(\/\/)(?![ \t]*-->)[^\n]*/m",
                ...
                function ($matches) {
                    return $this->StoreComment($matches[0]);
                }, $contents);
        }
    ...
    {
        $contents = preg_replace_callback('/___HANY_COMMENT_(\d+)/', function ($matches) {
            return $this->FetchComment($matches[1]);
        }, $contents);
